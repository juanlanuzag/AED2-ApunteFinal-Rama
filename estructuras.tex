...
\section{\'Arbol binario de b\'usqueda (ABB)}
...
La estrategia general para eliminar un nodo $u$ de un \'arbol binario de b\'usqueda $T$ tiene $3$ casos :

\begin{itemize}
 \item Si $u$ no tiene hijos, entonces simplemente lo removeremos modificando su padre $z.p$ reemplazando el puntero a $u$ por $NIL$.
 \item Si $u$ solo tiene un hijo, entonces subimos a dicho hijo a la posici\'on de $u$ modificando el padre $u.p$ reemplazando el puntero a $u$ por el puntero al \'unico hijo de $u$.
 \item Si $u$ tiene dos hijos:
      \begin{enumerate}
        \item Buscamos el predecesor inmediato $p$ al nodo a borrar $u$, que tiene seguro no tiene dos hijos, por ser predecesor inmediato.
        \item Copiamos la clave de $p$ en el lugar de la de $u$
        \item Borramos el nodo $p$ (que es hoja o tiene un hijo izquierdo)
      \end{enumerate}
\end{itemize}

\begin{SCfigure}[1][ht!]
 \centering
 \includegraphics[width=0.7\textwidth]{graficos/abb-borrado2h.png}
\end{SCfigure}

El costo del borrado es $O(h)$, con h la altura del arbol. Es el costo de encontrar el predecesor inmediato del nodo a borrar (que en el camino lo encontras) $O(h)$ y $O(1)$ los reacomodamientos de punteros y copia de claves.

\subsection{Balanceo perfecto}

Vimos que los ABB funcionan razonablemente bien en el caso promedio, pero no dan garant\'ias. Todos los algoritmos que vimos tienen un peor caso lineal. En \'este sentido, los arreglos tambi\'en son lineales en el peor caso, y ocupan menos memoria.
...
\section{\'Arbol balanceado en altura (AVL)}
...
\subsection{Peor caso en eliminaci\'on}
...
\begin{itemize}
 ...
 \item Las operaciones sobre las componentes de las claves pueden no ser f\'aciles.
\end{itemize}
...
\subsection{Posibles implementaciones}
\textbf{Primera implementaci\'on:} cada nodo interno contiene un arreglo de punteros en el que cada \'indice representa un caracter (o la parte de la clave). Esta implementacion resulta muy eficiente en t\'erminos de tiempo y sus algoritmos son simples, pero son muy ineficientes en terminos de memoria.
\textbf{Segunda Implementaci\'on:} los hijos de un nodo est\'an representados por una lista encadenada de la tupla <caracter a hijo, puntero al hijo>. Resulta eficiente en t\'erminos de tiempo s\'olo si hay pocas claves y requiere algoritmos de listas, pero es mucho m\'as eficiente en t\'erminos de memoria.

\subsection{Tries compactos}
Conciste en colapsar las cadenas de caracteres o bits que llevan hacia hojas.
Ejemplo: en un trie con dos claves ``Algo'' y ``Algoritmos'', el camino que lleva desde la primer `o' hasta la `s' puede ser compactado en un solo nodo para ahorrar espacio. Siguiendo la primera implementaci\'on pasa de tener 6 arreglos de punteros con una sola posici\'on distinta de null a tener solo una.
...
Ejemplo: en un trie con dos claves ``Algoritmo'' y ``Algoritmos'', el camino que lleva desde el nodo `A' hasta la \'ultima `o' puede ser compactado en un solo nodo para ahorrar espacio. Adem\'as la altura de un arbol Patricia queda acotado tambi\'en por el n\'umero de claves.
...
Son una implementaci\'on de una cola de prioridad.
\textbf{Invariante de Representaci\'on} (para Max-heap):
\begin{itemize}
  \item Es un arbol binario perfectamente balanceado
  \item Es izquierdista (el \'ultimo nivel est\'a completo desde la izquierda)
  \item La clave de cada nodo es mayor o igual que la de sus hijos, si los tiene.
  \item Todo subarbol es un heap.
\end{itemize}

Las dos implementaciones posibles son: \\
\textbf{Primera Implementaci\'on:} Representaci\'on con punteros. M\'as din\'amico pero ineficiente en t\'erminos de espacio. \\
\textbf{Segunda Implementaci\'on:} Representaci\'on con arreglos. A continuaci\'on explicaremos esta. \\

En la implementaci\'on con arreglos, el nodo v es almacenado en la posici\'on p(v) del arreglo. Si v es la raiz, p(v)=0 , si v es el hijo izquierdo de u, $p(v)= 2p(u)+1$, y si es hijo derecho, $p(v)= 2p(u)+2$. Por otro lado, $padre(v)= \lfloor (i-1)/2 \rfloor$ = $(i-1)>>1$.
Esta implementaci\'on es muy eficiente en t\'erminos de espacio y es muy acil de navegar, pero como contrapartida tiene una implementaciones est\'atica (puede ser necesario duplicar o achicar el arreglo a medida que se agregan y eliminan elementos). En general se implementan sobre vectores.

\subsection{M\'aximo}
Para obtener el m\'aximo solo debemos retornar lo que hay en la ra\'iz del arbol (para arreglos, lo que hay en la posici\'on 0). $O(1)$.

\subsection{Encolar}
Para Encolar un elemento e, lo insertamos al final de nuestro heap (en la \'ultima final, en el primer nodo libre desde la izquierda) y luego debemos restaurar el invariante del heap. El \'unico elemento que lo rompe es el recien insertado. Llamamos a la rutina Subir(e) que va subiendo el elemento en el arbol hasta restaurar el invariante.

Subir(e): \\
\indent \indent  while ! esRaiz(e) $\wedge$ (prioridad(e) $>$ prioridad(padre(e))): \\
\indent \indent \indent  swap(e, padre(e)) \\

El costo de este algoritmo en peor caso es O(log n), que es la altura del arbol.

\subsection{Desencolar}
Para Desencolar debemos eliminar la raiz del arbol. Para esto primeramente escribimos la raiz lo que hay en la \'ultima hoja y eliminamos la misma. En este momento, el \'unico nodo que no respeta el invariante es la ra\'iz. Para retaurarlo llamamos a la rutina Bajar(raiz(heap)).

Bajar(e): \\
\indent \indent while( ! isLeaf(e) $\wedge$ (prioridad(e) $<$ prioridad(algun hijo de p)))
\indent \indent \indent swap(e, hijoDeMayorPrioridad)

El costo de este algoritmo en peor caso es O(log n), que es la altura del arbol.

\subsection{Algoritmo de Floyd}
Dado un arreglo, buscamos convertirlo en un heap. La soluci\'on simple es ir encolando un un heap vac\'io cada uno de los elementos del arreglo.

El algoritmo de Floyd, en cambio, est\'a basado en la idea de aplicar la operacion bajar(e) a cada uno de los nodos desde abajo hacia arriba (bottom-up). Es decir, vamos heapificando cada uno de los subarboles.

heapify() \\
\indent \indent $for(i = len(heap) >> 1; i >= raiz(); i--) Bajar(i);$ \\

Notese que empezamos desde la mitad, pues la segunda mitad son todas hojas.
El costo en peor caso de este algoritmo es $O(n)$.

\subsubsection{Demostraci\'on}
Demostraci\'on de que el costo temporal del algoritmo de Floyd en peor caso es O(n).

El peor caso posible es que en cada llamada a Bajar(e) se hace el m\'aximo n\'umero de intercambios. Una llamada a bajar para un nodo de nivel i provoca como m\'aximo k-i intercambios.
Suponiendo un heap con $n = 2^k -1 $ nodos (un arbol binario completo de altura k),
\begin{itemize}
  \item En en \'ultimo nivel hay $(n+1)/2$ hojas
  \item En en pen\'ultimo nivel hay $(n+1)/4$ nodos
  \item En en antepen\'ultimo nivel hay $(n+1)/8$ nodos
  \item En el i-esimo hay $2^{i-1} = (n+1)/2^{k-i+1}$ nodos
\end{itemize}

Contando la m\'axima cantidad de intercambios:

$
  \#max\_intercambios = \\
  \indent \indent \indent (\#nodos en penultima fila) * 1 + \\
  \indent \indent \indent (\#nodos en antepenultima fila) * 2 + \\
  \indent \indent \indent ... + \\
  \indent \indent \indent (\#nodos en el nivel 2) * (k-2) + \\
  \indent \indent \indent (\#nodos en el nivel 1) * (k-1)
$

O lo que es lo mismo a: (con $k=lg(n+1)$)

$
  \#max\_intercambios = \\
  \indent \indent \indent ((n+1)/4) * 1 + \\
  \indent \indent \indent ((n+1)/8) * 2 + \\
  \indent \indent \indent ... + \\
  \indent \indent \indent (2) * (lg(n+1)-2) + \\
  \indent \indent \indent (1) * (lg(n+1)-1)
$

$$
\sum_{i=2}^{log(n+1)=k} \frac{n+1}{2^i} * (i-1) = (n+1) * \sum_{i=2}^{log(n+1)=k} \frac{i-1}{2^i}
$$

Y sabemos que $ \sum_{i=2}^{\infty} \frac{i-1}{2^i} $ converge a una constante c as\'i que:

$$
(n+1) * \sum_{i=2}^{log(n+1)=k} \frac{i-1}{2^i} < (n+1) * c
$$

Por lo tanto, \#max\_intercambios $= O(n) $

Tambi\'en sabemos que $ \sum_{i=2}^{log(n+1)=k} \frac{i-1}{2^i} > 0$ as\'i que:

$$
(n+1) * \sum_{i=2}^{log(n+1)=k} \frac{i-1}{2^i} > (n+1) * d
$$

Por lo tanto, \#max\_intercambios $= \Omega(n) $ entonces \#max\_intercambios $= \Theta(n) $
...
\begin{itemize}
 \item Direccionamiento cerrado (resoluci\'on por concatenaci\'on): a la i\-\'esima posici\'on de la tabla se le asocia la lista de los elementos tales que $h(k)=i$
 \item Direccionamiento abierto (resolucii\'on por rehashing): todos los elementos se guardan en la tabla.
\end{itemize}
...
Pero es muy d\'ificil construir funciones que satisfagan Uniformidad Simple, ya que $P$ es generalmente desconocida. Por lo tanto en la pr\'actica se trata de tener independencia de la distribuci\'on de los datos.
...
\item Con $n=\#$elementos en la tabla, y $\alpha = n / |N|$ (factor de carga).
\end{itemize}

Por lo que sí $n \sim |N|$, obtenemos $O(1)$. Lo que haremos es ir redimencionando el arreglo de acuerdo al factor de carga, para mantenerlos controlado.

\subsection{Direccionamiento abierto}
A diferencia del Direccionamiento cerrado, todos los elementos se almacenan en la tabla, y las colisiones se resuelven dentro de la tabla.
...
Es decir, $(\forall i) (h'(K_1) == h'(K_2)) \Rightarrow (h(K_1,i) == h(K_2,i)) $
\end{itemize}

\subsubsection{Barrido lineal (linear probing)}
...
\subsubsection{Barrido cuadr\'atico (quadriatic probing)}
Usamos:
...
Se busca que el barrido depende tambi\'en de la clave:
$$h(k,i) = (h_1(k) + i * h_2(k))\ mod\ |T|$$ donde $h_1(k)$ y $h_2(k)$ son funciones de hashing.
Reduce los fen\'omenos de aglomeraci\'on secundaria, y no tiene aglomeraci\'on primaria.

\subsection{Funci\'on de Hash}
Lo que buscamos de una funci\'on de hash es obtener una distribuci\'on lo m\'as uniforme posible.
\subsubsection{Divisi\'on}
$h(k) = k mod |T|$

\textbf{Desventaja:} si tenemos|T|$=2^{p}$ entonces todas las clave con los bits menos significativos iguales colisionan. Lo mismo ocurre con los numeros decimales y las potencias de 10. Debemos buscar que la funci\'on de hash dependa de todas las cifras de la clave, cualquiera sea su representacion. Por lo tanto resulta una buena elecci\'on un n\'umero primo no demasiado cercano a una potencia de 2.

\subsubsection{Partici\'on}
Consiste en particionar la clave k en $ k_{1},k_{2},...,k_{n} $ y calcular $ h(k) = h'(k_{1},k_{2},...,k_{n}) $

\subsubsection{Extracci\'on}
Se usa solamente una parte de la clave para calcular la direcci\'on.
...
\subsection{Optimalidad de los Splay trees}
\begin{itemize}
...
\subsection{Max/Min, Inserci\'on y Eliminaci\'on}
...
\subsection{Inserci\'on en Arboles 234}
...
\subsection{Eliminaci\'on en Arboles 234}
...
\subsection{Lema (cota de altura)}
...
\subsection{B\'usqueda, m\'aximo y m\'inimo}
